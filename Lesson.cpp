...
Lesson::Lesson(int font, Teacher* teacher_p, Stats* stats_p, Stats* dailyStats_p) {
	...
	m_morningReviewButton = new Button("振り返り", 900, 600, 350, 100, LIGHT_BLUE, BLUE, m_font, BLACK);
	...
	m_onlyImportantSpeakingStudyButton = new Button("重要文のみ音読", 900, 750, 350, 100, LIGHT_BLUE, BLUE, m_font, BLACK);
...
bool Lesson::play(int handX, int handY, int mouseWheel) {
	...
	switch (m_state) {
		...
		case SPEAKING_STUDY:		// 音読練習
			...
			break;
		case SPEAKING_STUDY_IMPORTANT:		// 重要文音読
...
void Lesson::draw(int handX, int handY) const {
	...
	case SPEAKING_STUDY:		// 音読練習
		...
		lessonName = "音読練習の時間 20m";
		break;
	case SPEAKING_STUDY_IMPORTANT:		// 重要文音読
		m_speakingPractice->draw(handX, handY);
		lessonName = "重要文音読の時間 20m";
		break;
	}
